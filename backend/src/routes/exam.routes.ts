...
router.post(
  ...
  async (req: AuthRequest, res) => {
    try {
      const exam = await ExamService.createExam(req.user!.userId, {
...
// Get exam
router.get('/exams/:examId', authenticate, async (req: AuthRequest, res) => {
  try {
    const isTutor = req.user?.role === 'tutor' || req.user?.role === 'admin';
    const exam = await ExamService.getExamById(req.params.examId, isTutor);
    res.json(exam);
  } catch (error: any) {
...
router.post(
  ...
  async (req: AuthRequest, res) => {
    try {
      const result = await ExamService.gradeSubmission(
